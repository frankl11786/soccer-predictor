...
class SnapshotTests(unittest.TestCase):
    def test_snapshots(self):
        for league, expected_min in (("epl", 18), ("mls", 28)):
            ...
            modern_market_schema = "market-comparison" in str(data.get("meta", {}).get("model_version", ""))
            if modern_market_schema:
                market_meta = data.get("meta", {}).get("polymarket", {})
                self.assertIsInstance(market_meta, dict)
                self.assertTrue(data.get("methodology", {}).get("market_comparison", {}).get("comparison_only"))

                team_by_slug = {team["slug"]: team for team in data["teams"]}
                ...
                for row in data["forecast"]:
                    ...
                    if interval and effect_interval:
                        self.assertAlmostEqual(float(interval[0]), -float(effect_interval[1]), delta=0.00001)
                        self.assertAlmostEqual(float(interval[1]), -float(effect_interval[0]), delta=0.00001)
            ...
            for fixture in data["fixtures"]:
                ...
                raw_probs = market["raw_probabilities"]
                edges = market["model_edge"]
                self.assertEqual(set(market_probs), {"home", "draw", "away"})
                self.assertAlmostEqual(sum(float(value) for value in market_probs.values()), 1.0, delta=0.00001)
                self.assertAlmostEqual(sum(float(value) for value in raw_probs.values()), float(market["normalization_total"]), delta=0.00001)
                for result in ("home", "draw", "away"):
                    self.assertAlmostEqual(float(edges[result]), float(probs[result]) - float(market_probs[result]), delta=0.00001)
                self.assertTrue(market.get("comparison_only"))
                self.assertTrue(market.get("event_slug") or market.get("event_id"))
